...
if (isset($_GET['post_id']) && isset($_GET['user_id'])) {
    ...
    $check_sql = "SELECT * FROM likes WHERE esp_id = $user_id AND post_id = $post_id";
    $check_result = $conexao->query($check_sql);

    if ($check_result->num_rows > 0) {
        // O usuário já curtiu a postagem, então remova a curtida
        $delete_sql = "DELETE FROM likes WHERE esp_id = $user_id AND post_id = $post_id";
        ...
        $insert_sql = "INSERT INTO likes (esp_id, post_id) VALUES ($user_id, $post_id)";
